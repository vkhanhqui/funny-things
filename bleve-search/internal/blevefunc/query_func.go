...
func parseSortOrder(sortPart map[string]interface{}) (search.SortOrder, error) {
	...
	if !ok {
		return nil, fmt.Errorf("parseSortOrder: missing or invalid sort field")
	}
...
func ExtractQueries(searchRequest *bleve.SearchRequest, queries []map[string]interface{}) error {
	...
	for _, queryData := range queries {
		...
		if !ok {
			return fmt.Errorf("extractQueries: invalid query format: got %v", queryData["query"])
		}
...
func ExtractFields(searchRequest *bleve.SearchRequest, queries []map[string]interface{}) {
	var fields []string
...
func ExtractPagination(searchRequest *bleve.SearchRequest, queries []map[string]interface{}) {
	for _, queryData := range queries {
...
func ExtractMiscellaneous(searchRequest *bleve.SearchRequest, queries []map[string]interface{}) {
	for _, queryData := range queries {
	...
	for _, queryData := range queries {
		if facetsPart, ok := queryData["facets"].(map[string]interface{}); ok {
			for facetName, facetData := range facetsPart {
				...
				if err != nil {
					return fmt.Errorf("extractFacets: failed to marshal facet data for %s: %w", facetName, err)
				}

				var facetRequest bleve.FacetRequest
				if err := json.Unmarshal(facetJSON, &facetRequest); err != nil {
					return fmt.Errorf("extractFacets: failed to unmarshal facet request for %s: %w", facetName, err)
				}
...
func ExtractSortOrders(searchRequest *bleve.SearchRequest, queries []map[string]interface{}) error {
	...
	for _, queryData := range queries {
		if sortPart, ok := queryData["sort"].(map[string]interface{}); ok {
			...
			if err != nil {
				return fmt.Errorf("extractSortOrders: failed to parse sortOrder: got %v and err %w", sortPart, err)
			}
